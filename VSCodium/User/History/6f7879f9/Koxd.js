document.addEventListener('DOMContentLoaded', () => {
    ...
    // Run dynamic node listeners on page startup to capture initialization drags
    ...
    // Direct Inline Style Overrider to solve internal thumb node clipping locks
    function rotateShurikenKnob(inputElement) {
        ...
        // Injects directly over the active element root tree to force browser frame redraws
        inputElement.style.setProperty('--thumb-transform', `rotate(${calculatedDegrees}deg)`);
    }
    ...
    function killActiveAudioEngine() {
        ...
        document.querySelectorAll('.timeline-slider').forEach(s => {
            s.value = 0;
            s.style.setProperty('--thumb-transform', 'rotate(0deg)');
        });
